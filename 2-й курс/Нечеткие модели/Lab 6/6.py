...
data = pd.DataFrame({
    "Вік клієнта": ['молодий', 'молодий', 'середній', 'похилий', 'похилий', 'похилий', 'середній', 'молодий', 'молодий', 'похилий', 'молодий',
                      'середній', 'середній', 'похилий'],
    "Дохід клієнта": ['великий', 'великий', 'великий', 'середній', 'невеликий', 'невеликий', 'невеликий', 'середній', 'невеликий', 'середній',
                    'середній', 'середній', 'великий', 'середній'],
    "Чи є студентом": ['-', '-', '-', '-', '+', '+', '+', '-', '+', '+', '+', '-', '+', '-'],
    "Кредитний рейтинг": ['гарний', 'дуже гарний', 'гарний', 'гарний', 'гарний', 'дуже гарний', 'дуже гарний', 'гарний', 'гарний', 'гарний', 'дуже гарний', 'дуже гарний', 'гарний', 'дуже гарний'],
    "Чи купив комп'ютер": ['-', '-', '+', '+', '+', '-', '+', '-', '+', '+', '+', '+', '+', '-']
})
...
class CART_classifier():
    ...
    def getMinGini(self, data, col, control_col=-1):
        ...
        for k in np.unique(data.iloc[:, col]):
            ...
            new_gini = part_rows / total_rows * self.calc_gini(part_data.iloc[:, control_col]) + \
                       left_rows / total_rows * self.calc_gini(left_data.iloc[:, control_col])
            if new_gini < gini:
                gini = new_gini
                best_k = k
        return gini, k
